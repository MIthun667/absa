...
def main():
    ...
    # ------------------------------------------------------------
    # Initialize all existing Task-3 parameters from the strongest
    # hybrid checkpoint. Pair-specific modules are new and remain
    ...
    print(
        "Loaded frozen hybrid checkpoint:",
        hybrid_checkpoint_path,
    )

    print(
        "source checkpoint step:",
        hybrid_checkpoint["step"],
    )
    ...
    # ------------------------------------------------------------
    # Freeze the proven base model completely.
    # Only the pair-category modules are trainable.
    # ------------------------------------------------------------
    ...
    for step in range(
        ...
        # --------------------------------------------------------
        # Frozen base remains deterministic.
        # Only the new pair-category modules stay in train mode.
        # --------------------------------------------------------

        model.encoder.eval()

        model.aspect_classifier.eval()
        model.opinion_classifier.eval()

        model.null_aspect.eval()
        model.null_opinion.eval()

        model.relation_encoder.eval()
        model.relation_classifier.eval()

        model.category_classifier.eval()
        model.entity_classifier.eval()
        model.attribute_classifier.eval()

        model.category_embeddings.eval()
        model.va_encoder.eval()
        model.va_regressor.eval()

        # Explicitly keep new pair modules trainable.
        model.entity_embeddings.train()
        model.attribute_embeddings.train()
        model.category_composer.train()
        model.relation_category_projection.train()
        ...
        with amp_context(
            ...
            # ------------------------------------------------
            # Frozen-base compositional training.
            #
            # Do not allow the pair objective to disturb the
            # successful Task-3 representation.
            # ------------------------------------------------

            loss = pair_category_loss
    ...
    # ------------------------------------------------------------
    # Frozen-pair training:
    # flat-only dev cF1 is intentionally constant because the
    # successful base model is frozen. Therefore also persist the
    # FINAL pair scorer, rather than selecting the pair branch by
    # flat-only cF1.
    # ------------------------------------------------------------

    final_pair_checkpoint = (
        output_dir
