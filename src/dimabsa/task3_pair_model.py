...
class Task3PairModel(nn.Module):
    def __init__(
        self,
        model_name: str,
        *,
        ...
        # ---------------------------------------------------------
        # Learned compositional category prototypes.
        #
        # A category is represented from its reusable Entity and
        # Attribute factors rather than by an unrelated output
        # vector alone.
        # ---------------------------------------------------------

        self.composition_dim = 128
        ...
        self.category_composer = nn.Sequential(
            nn.Linear(
                self.composition_dim * 3,
                self.composition_dim,
            ),
            nn.GELU(),
            nn.Dropout(dropout),
        ...
        self.relation_category_projection = nn.Sequential(
            nn.Linear(
                hidden,
                self.composition_dim,
            ),
    ...
    def pair_category_logits(
        self,
        relation_repr: torch.Tensor,
        category_entity_indices: torch.Tensor,
        category_attribute_indices: torch.Tensor,
    ) -> torch.Tensor:

        entity_repr = self.entity_embeddings(
            category_entity_indices
        )

        attribute_repr = self.attribute_embeddings(
            category_attribute_indices
        )
        ...
        relation_projection = (
            self.relation_category_projection(
                relation_repr
            )
        )
